        ...
        <table align="center" border="0" width="750" cellpadding="0"
          ...
          <c:choose>
            ...
            <c:otherwise>
                  ...
                  <td><!-- 留言板内容 -->
                    <table border="0" width="750" align="center" cellpadding="0"
                      ...
                      <tr bordercolor="#FFFFFF" bgcolor="#F0F0F0">
                        <td height="22"><!-- 留言标题 -->
                          <font class="title1">【${m.title}】</font></td>
                        <td align="right"><!-- 判断管理员用户是否登录 -->
                          <c:if test="${!empty admin}">
                            <c:if test="${empty m.revert}"> <a href="ManagerServlet?method=revert&id=${m.id}">回复</a> </c:if>
                            <a href="ManagerServlet?method=delete&id=${m.id}">删除</a> </c:if></td>
                      </tr>
                      ...
                      <tr bordercolor="#FFFFFF" bgcolor="#FFFFFF">
                        <td colspan="2" align="right" class="td2"> 网友：${m.user.username}&nbsp;|&nbsp; 
                          时间：
                          <fmt:formatDate pattern="yyyy-dd-MM HH:mm:ss"
														value="${m.createTime}" />
                          &nbsp;|&nbsp; 
                          联系方式： <a href="mailto:${m.user.email}">${m.user.email}</a></td>
                      </tr>
                      <!-- 判断是否存在回复信息 -->
                      <c:if test="${!empty m.revert.content}">
                        <!-- 输出回复信息 -->
                        <tr>
                          <td colspan="2"><div class="hf" align="center">
                              <table border="0" cellpadding="1" cellspacing="1" width="690">
                                <tr>
                                  <td align="left" valign="middle"><img src="images/admin.jpg" width="13" height="18"> <font class="hf-title">管理员回复：</font></td>
                                  <td align="right"><!-- 判断是否是管理登录 -->
                                    <c:if test="${!empty admin}"> <a href="ManagerServlet?method=revert&id=${m.id}">修改</a> </c:if></td>
                                </tr>
                                <tr>
              ...
              <!-- 分页条 -->
              <tr bordercolor="#FFFFFF" bgcolor="#FFFFFF">
                <td colspan="2"><table border="0" width="750" align="center">
                    <tr>
                      <td align="right"> 总记录数：${pageModel.totalRecords}
                        当前${pageModel.currPage}/${pageModel.totalPage}页 <a href="MessageServlet?method=view&currPage=${pageModel.previousPage}"> 上一页 </a> <a href="MessageServlet?method=view&currPage=${pageModel.nextPage}"> 下一页 </a>
